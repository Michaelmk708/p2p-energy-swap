...
export default function Marketplace() {
  ...
  const [selectedListing, setSelectedListing] = useState<typeof marketListings[0] | null>(null);
  ...
  const handleMpesaPayment = () => {
    ...
    toast.success(`M-Pesa payment request sent to ${phoneNumber}. Check your phone to complete payment.`);
    
    // Simulate successful payment after a delay
    setTimeout(() => {
      addNotification({
        type: "buy",
        message: `You bought ${selectedListing.tokens} tokens from ${selectedListing.seller} for KES ${(selectedListing.tokens * selectedListing.pricePerToken).toFixed(2)}`,
      });
              ...
              <div className="space-y-4 pt-4">
                <div className="p-4 rounded-lg bg-muted">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="font-semibold">{selectedListing.tokens}</span>
                  </div>
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-sm text-muted-foreground">Price per token</span>
                    <span className="font-semibold">KES {selectedListing.pricePerToken}</span>
                  </div>
                  <div className="h-px bg-border my-2" />
                  <div className="flex justify-between items-center">
                    <span className="text-sm font-semibold">Total Amount</span>
                    <span className="text-xl font-bold text-primary">
                      KES {(selectedListing.tokens * selectedListing.pricePerToken).toFixed(2)}
                    </span>
                  </div>
                </div>
                ...
                <div className="p-3 rounded-lg bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800">
                  <p className="text-xs text-blue-900 dark:text-blue-100">
                    <strong>Note:</strong> M-Pesa integration is currently in development. 
